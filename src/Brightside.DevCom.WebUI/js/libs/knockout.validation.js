...
(function(factory) {
    ...
    var brightside = (function() {
        return {
            clearValidations: function(model) {
                for (var i in model) {
                    if (model[i].clearError) {
                        model[i].clearError();
                    }
                }
            },
            rebindValidations: function(model, errors) {
                for (var i in model) {
                    if (model[i].clearError) {
                        model[i].clearError();
                        $.each(errors, function(index, item) {
                            if (item.PropertyName.toLowerCase() == i) {
                                model[i].setError(item.ErrorMessage);
                            }
                        });
                    }
                }
            ...
            isValid: function(model) {
                ...
                for (var i in model) {
                    if (model[i].clearError) {
                        ko.validation.validateObservable(model[i]);
    ...
    ko.utils.extend(validation, brightside);
    //#endregion
